...
function App(Props) {
  ...
  const groupId = "321-123"
  const FB = firebase.firestore().collection("messages").doc("321 - 123").collection("321 - 123");
  console.log("fb")
  console.log(FB)
  const scroll = useRef()
  const d1 = ""+Date.now()
  ...
  const HandelAddDataBase =() =>{
    ...
    firebase.firestore().collection("messages").doc("321 - 123").collection("321 - 123").doc(d1).set({
      senderId:'123',
      anotherUserId:'123',
      timestamp:d1,
      content:texting,
...
setTexting(event.target.value)
  ...
  return (
    ...
    <div style={{width:'100%',height:50,fontFamily: "Markazi Text",
                      fontSize: "30px",display:'flex',justifyContent:'center'}}>{first}</div>
